...
import { getEmployee, updateExist } from "../api";
...
const EmployeeDetails = () => {
  ...
  const [updateExistUser,setUpdateExistUser] = useState(false)
  ...
  useEffect(() => {
    ...
    fetchEmployees();
    setUpdateExistUser(false)
  }, [token, currentUser, dispatch, reload, employeeId,updateExistUser]);
  ...
  const updateExistingAction = async (id,active) =>{
      if(active=="Active"){
         try {
          await updateExist({id:id,active:false},token).then(res=>{
            setUpdateExistUser(true);
          }).catch(err=>{
            console.log(err)
          })
         } catch (error) {
        ...
        try {
          await updateExist({id:id,active:true},token).then(res=>{
             setUpdateExistUser(true);
          }).catch(err=>{
             console.log(err)
          })
                    ...
                    <Tags>
                      ...
                      {employee.active ? (
                        <Tag
                          ...
                          onClick={()=>updateExistingAction(employee._id,"Active")}
                        >
                          Active
                        </Tag>
                      ) : (
                        <Tag
                          ...
                          onClick={()=>updateExistingAction(employee._id,"Deactivated")}
                        >
                          Deactivated
                        </Tag>
                      )}
